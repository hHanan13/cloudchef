...
class DashboardStatistics extends Component
{
    ...
    public function top_pages(){
        ...
        })->whereNotIn('url',['/admin/notifications/ajax',route('manifest')])->groupBy('url')->orderBy(\DB::raw("count('url')"),'DESC')->select()->addSelect(\DB::raw("count('url') as count"))->limit(10)->get();
    }
