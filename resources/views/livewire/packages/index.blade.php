            ...
            <div class="card-header card-header-primary card-header-icon">
               <div class="row">
                  <div class="col-md-6 col-sm-12">
                     ...
                     <h4 class="card-title">@lang('lang.packages')
      ...
      <div class="card-body responsive_padding">
         ...
         <div class="material-datatables table-responsive table">
            <table id="datatables" class="table table-striped table-no-bordered table-hover"
               ...
               <thead>
               @if($data['packages'] !== 'null' && count($data['packages'] )>0)
                            <tr>
                                <th>@lang('lang.id')</th>
               ...
               @foreach($data['packages']  as $package)
                            <tr>
                                ...
                                <th>{{$package->name}}</th>
                                <th>{{$package->price}}</th>
                                <th>{{isset($package->created_at) ?$package->created_at->diffForHumans() :''}}

                                </th>
                                <td>
                                <a class="btn"><label class="switch tog">
                                            <input type="checkbox" onclick="confirm('Are you sure?') || event.stopImmediatePropagation()" 
                                            wire:click="activatePackage({{$package->id}})" {{$package->status == true ? 'checked' : ' '}}>
                                ...
                                <td class="text-right">
                                    <a data-toggle="modal" onclick="RenderActions1('')"
                                       data-backdrop="static" data-keyboard="false" wire:click="toggleEditModal({{$package->id}})"
                                       title="Click to edit"
                                       class="btn" > <i
                                    ...
                                    <a onclick="confirm('Are you sure?') || event.stopImmediatePropagation()"
                                       wire:click="destroy({{$package->id}})" class="btn" ><i
                                            class="material-icons"><img
                                    ...
                                    @endcan
                                        ...
                                        @can('read_packages')
                                    <a data-toggle="modal" wire:click="toggleShowModal({{$package->id}})"
                  ...
                  @if($EditModelOpened==true)
                            @include('livewire.packages.edit')

                            {{--// showpopModal--}}
                        @elseif($ShowModelOpened==true)

                            @include('livewire.packages.show')
