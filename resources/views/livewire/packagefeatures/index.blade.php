            ...
            <div class="card-header card-header-primary card-header-icon">
               <div class="row">
                  <div class="col-md-6 col-sm-12">
                     ...
                     <h4 class="card-title">@lang('lang.packagefeatures')
      ...
      <div class="card-body responsive_padding">
         ...
         <div class="material-datatables table-responsive table">
            <table id="datatables" class="table table-striped table-no-bordered table-hover"
               ...
               <thead>
               @if($data['packagefeatures'] !== 'null' && count($data['packagefeatures'] )>0)
                        <tr>
                            <th>@lang('lang.id')</th>
               ...
               <tbody>
               @foreach($data['packagefeatures']  as $packagefeature)
                        <tr>
                            <td><input type="checkbox"
                                       aria-label="Checkbox for following text input"></td>
                            <th>{{$packagefeature->name}}</th>
                            <th>{{isset($packagefeature->created_at) ?$packagefeature->created_at->diffForHumans() :''}}

                            </th>
                            <td>
                            <a><label class="switch tog">
                                    <input type="checkbox" onclick="confirm('Are you sure?') || event.stopImmediatePropagation()" 
                                    wire:click="activatePackageFeature({{$packagefeature->id}})" {{$packagefeature->status == true ? 'checked' : ' '}}>
                            ...
                            <td class="text-right">
                                ...
                                @can('update_packagefeatures')

                                    <a data-toggle="modal" onclick="RenderActions1('')"
                                       data-backdrop="static" data-keyboard="false"
                                       wire:click="toggleEditModal({{$packagefeature->id}})"
                                       title="Click to edit"
                                ...
                                @can('delete_packagefeatures')

                                    <a onclick="confirm('Are you sure?') || event.stopImmediatePropagation()"
                                       wire:click="destroy({{$packagefeature->id}})" class="btn" style="padding: 5px;"><i
                                            class="material-icons"><img
                            ...
                            <!--  <a class="btn remove" rel="tooltip" href="javascript:void(0);"
                                                   wire:click="destroy({{$packagefeature->id}})">
                                ...
                                @can('read_packagefeatures')
                                    ...
                                    <a data-toggle="modal" wire:click="toggleShowModal({{$packagefeature->id}})"
                                       data-backdrop="static" data-keyboard="false"
                  ...
                  @if($EditModelOpened==true)
                        @include('livewire.packagefeatures.edit')

                        {{--// showpopModal--}}
                    @elseif($ShowModelOpened==true)

                        @include('livewire.packagefeatures.show')


                        <!-- createpopModal -->
                    @elseif($AddModelOpened==true)

                        @include('livewire.packagefeatures.create')
                    @endif
...
<!-- end row -->
<!-- /.box -->
@if(count($data['packagefeatures'] )>0)
    <div class="align-center">
        {!! $data['packagefeatures']->links()  !!}
    </div>
    @endif
