            ...
            <div class="card-header card-header-primary card-header-icon">
               <div class="row">
                  <div class="col-md-6 col-sm-12">
                     ...
                     <h4 class="card-title">@lang('lang.services')
      ...
      <div class="card-body responsive_padding">
         ...
         <div class="material-datatables table-responsive table">
            <table id="datatables" class="table table-striped table-no-bordered table-hover"
               ...
               <thead>
               @if($data['services'] !== 'null' && count($data['services'] )>0)
                            <tr>
                                <th>@lang('lang.id')</th>
               ...
               @foreach($data['services']  as $service)
                            <tr>
                                ...
                                <th>{{$service->name}}</th>
                                <th>{{isset($service->created_at) ?$service->created_at->diffForHumans() :''}}
                                ...
                                <td><span><img
                                            ...
                                            wire:click="activateService({{$service->id}})" {{$service->status == true ? 'checked' : ' '}}>
                                ...
                                <td class="text-right">
                                    ...
                                    <a data-toggle="modal" onclick="RenderActions1('')"
                                       data-backdrop="static" data-keyboard="false" wire:click="toggleEditModal({{$service->id}})"
                                       title="Click to edit"
                                    ...
                                    <a onclick="confirm('Are you sure?') || event.stopImmediatePropagation()"
                                       wire:click="destroy({{$service->id}})" class="btn" style="padding: 5px;"><i
                                            class="material-icons"><img
                                ...
                                <!--  <a class="btn remove" rel="tooltip" href="javascript:void(0);"
                                        ...
                                        @can('read_services')
                                    <a data-toggle="modal" wire:click="toggleShowModal({{$service->id}})"
                  ...
                  @if($EditModelOpened==true)
                        ...
                        @elseif($ShowModelOpened==true)
                            ...
                            @include('livewire.services.show')
